...
class Position {
 ...
 public:
  ...
  Color getSideToMove() const { return sideToMove; }  // Get the side to move
  ...
  void setPiece(Piece piece, Square sq) {
    ...
    byType[static_cast<PieceType>(piece % 6)] |=
        bb;  // Set the bit for this piece type
  }

  void removePiece(Piece piece, Square sq) {
    Bitboard bb = squareBB(sq);
    pieces[piece] &= ~bb;
    byColor[static_cast<Color>(piece / 6)] &=
        ~bb;  // Clear the bit for this color
    byType[static_cast<PieceType>(piece % 6)] &=
