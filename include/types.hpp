...
enum File : uint8_t {
  FILE_A, FILE_B, FILE_C, FILE_D, FILE_E, FILE_F, FILE_G, FILE_H, FILE_NONE = 8
};

enum Rank : uint8_t {
  RANK_1, RANK_2, RANK_3, RANK_4, RANK_5, RANK_6, RANK_7, RANK_8, RANK_NONE = 8
};

enum Color : uint8_t { WHITE, BLACK, COLOR_NONE = 2 };

enum PieceType : uint8_t {
  PAWN, KNIGHT, BISHOP, ROOK, QUEEN, KING, PIECE_TYPE_NONE = 6
};
...
// Board zone masks
constexpr Bitboard QUEENSIDE_BB        = FILE_BB[FILE_A] | FILE_BB[FILE_B] | FILE_BB[FILE_C] | FILE_BB[FILE_D];
constexpr Bitboard KINGSIDE_BB         = FILE_BB[FILE_E] | FILE_BB[FILE_F] | FILE_BB[FILE_G] | FILE_BB[FILE_H];
constexpr Bitboard CENTER_SQUARES_BB   = squareBB(D4) | squareBB(E4) | squareBB(D5) | squareBB(E5);
constexpr Bitboard EXTENDED_CENTER_BB  = squareBB(C3) | squareBB(D3) | squareBB(E3) | squareBB(F3) |
                                         squareBB(C4) | squareBB(D4) | squareBB(E4) | squareBB(F4) |
                                         squareBB(C5) | squareBB(D5) | squareBB(E5) | squareBB(F5) |
                                         squareBB(C6) | squareBB(D6) | squareBB(E6) | squareBB(F6);
